...
class LinearSVCClassifier(Classifier):
    ...
    def important_features(self,  feature_names, n=20):
        coefs_with_fns = sorted(zip(self.classifier.coef_[0], feature_names))
...
class RFClassifier(Classifier):
    ...
    def important_features(self, feature_names, target_column, threshold=0.05):
        ...
        for f in range(significant_feature_index.size):
            logger.debug("%d. feature %d %s (%f)", f + 1, significant_feature_index[f],
...
class ETClassifier(Classifier):
   def __init__(self):
       clf = ExtraTreesClassifier(n_jobs=8)
       super().__init__(clf)
